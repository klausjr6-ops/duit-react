...
export default function EditWalletModal({ wallet, onClose }: Props) {
  ...
  // NOTE: wallet.balance is derived (from walletsWithBalance: base balance + all non-CF tx net amount).
  // To recover the original "initial balance", we subtract net tx amount of ALL non-CF txs
  // (including goal and transfer txs, which also affect the derived balance).
  const netTxAmount = txs.filter(t => t.walletId === wallet.id && !t.isCarryForward).reduce((s, t) => s + (t.type === "in" ? t.amt : -t.amt), 0);
  ...
  const handleSubmit = async () => {
    ...
    setSaving(true);
    try {
      const result = await updateWallet(wallet.id, { name: name.trim(), icon, color: colorKey, balance: bal });
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={guardedClose} className="fixed inset-0 z-[70] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
      <motion.div ref={dialogRef} role="dialog" aria-modal="true" onKeyDown={onDialogKeyDown} initial={{ scale: 0.95, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95, y: 20 }} onClick={e => e.stopPropagation()} className={panel}>
        <div className="flex justify-between items-center mb-5"><h2 className={isDark ? "text-xl font-bold text-white" : "text-xl font-bold text-zinc-900"}>Edit Dompet</h2><button onClick={guardedClose} disabled={saving} className={`${closeCls} disabled:cursor-not-allowed disabled:opacity-40`}><IconClose size={20} /></button></div>
        <div className="space-y-4">
          <div><label className={labelCls}>Nama Dompet</label><input value={name} disabled={saving} onChange={e => setName(e.target.value)} className={inputCls} /></div>
          <div><label className={labelCls}>Icon</label>
            ...
            {!WALLET_ICONS.some((ic) => ic.key === icon) && (
              <div className="mt-2 flex items-center gap-2">
                ...
                <button
                  type="button"
                  onClick={() => setIcon(WALLET_ICONS[0].key)} disabled={saving}
              ...
              {WALLET_COLORS.map((c) => {
                ...
                return (
                  <button
                    ...
                    type="button"
                    title={c.label}
                    onClick={() => setColorKey(c.key)} disabled={saving}
          ...
          <div><label className={labelCls}>Saldo Awal</label>
            <input type="text" inputMode="numeric" value={initialBalance ? `Rp ${formatRp(initialBalance)}` : ""} disabled={saving} onChange={e => setInitialBalance(e.target.value.replace(/\D/g, ""))} className={inputCls} />
